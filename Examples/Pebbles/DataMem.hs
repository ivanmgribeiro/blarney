...
makeDataMemCore sim req = do
  ...
  always do
    let isUnaligned = select [ isWordAccess (req.width_memIn.val) --> slice @1 @0 (req.addr.val) .!=. 0
                             , isHalfAccess (req.width_memIn.val) --> at @0 (req.addr.val) .!=. 0
    ...
    when (readPending.val) do
      ...
      let hAligned = 0 # ((at @1 (req.addr.val) .==. 0) ? (b1 # b0, b3 # b2))
      -- TODO clean this up into a case statement?
      let bAligned = select [ slice @1 @0 (req.addr.val) .==. 0 --> 0 # b0
                            , slice @1 @0 (req.addr.val) .==. 1 --> 0 # b1
                            , slice @1 @0 (req.addr.val) .==. 2 --> 0 # b2
                            , slice @1 @0 (req.addr.val) .==. 3 --> 0 # b3
                            ]
      responseData <== select [
